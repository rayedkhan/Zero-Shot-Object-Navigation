...
class FMMPlanner():
    ...
    def set_multi_goal(self, goal_map, state):
        ...
        # traversible_ma[goal_map == 1] = 0 # what if goal is on mask?
        traversible_ma[goal_x, goal_y] = 0
        dd = skfmm.distance(traversible_ma, dx=1)
        
        dd = ma.filled(dd, np.max(dd) + 1) # fill untraversible place as max distance
        if dd[state[0],state[1]] == np.max(dd): # agent can not traverse to goal
    ...
    def _find_nearest_goal(self, goal, state=None):
        ...
        if state is not None:
            top_left = (min(goal[0],state[0]),min(goal[1],state[1]))
            down_right = (max(goal[0],state[0]),max(goal[1],state[1]))
            top_left_selected = (max(0,top_left[0]-80), max(0,top_left[1]-80))
            down_right_selected = (min(max_x, down_right[0]+80), min(max_y, down_right[1]+80))
        else:
        ...
        if state is not None:
            state = (state[0]-top_left_selected[0],state[1]-top_left_selected[1])
            planner.set_goal(state)
            dist_agent_map = planner.fmm_dist * mask
            dist_agent_map[dist_agent_map == 0] = dist_agent_map.max() * 2
            dist_sort_idx = np.argsort(dist_map+0.3*dist_agent_map, axis=None) # a little time cosuming 
        else:
